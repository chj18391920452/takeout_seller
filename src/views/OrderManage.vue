...
<script>
    export default {
        ...
        methods:{
            ...
            page(currentPage){
                const _this = this
                axios.get('http://localhost:8180/order-service/seller/order/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
            ...
            cancel(row) {
                const _this = this
                axios.put('http://localhost:8180/order-service/seller/order/cancel/'+row.orderId).then(function (resp) {
                    if(resp.data.code == -1){
            ...
            finish(row) {
                if(row.payStatus == 0){
                    ...
                    const _this = this
                    axios.put('http://localhost:8180/order-service/seller/order/finish/'+row.orderId).then(function (resp) {
                        if(resp.data.code == -1){
                            _this.$alert('订单【'+row.orderId+'】状态异常', '错误', {
                                confirmButtonText: '确定'
                            });
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8180/order-service/seller/order/list/1/'+_this.pageSize).then(function (resp) {
                _this.tableData = resp.data.data.content
                _this.pageSize = resp.data.data.size
                _this.total = resp.data.data.total
            })
