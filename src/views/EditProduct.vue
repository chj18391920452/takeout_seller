...
<script>
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put('http://localhost:8180/product-service/seller/product/update',_this.ruleForm).then(function(resp){
                            if(resp.data.code == 0){
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8180/product-service/seller/product/findAllProductCategory').then(function(resp){
                _this.category = resp.data.data.content
            })
            axios.get('http://localhost:8180/product-service/seller/product/findById/'+this.$route.query.id).then(function(resp){
